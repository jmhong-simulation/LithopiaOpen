...
public:
   ...
   {if(x_) delete[] x_;if(Q_) delete Q_;if(R_) delete(R_);if(V_) delete(V_);}
    ...
    int m1=min(m_,m_new),n1=min(n_,n_new);for(int i=0;i<m1;i++)for(int j=0;j<n1;j++) *(x_new+(j)*m_new+(i))=(*this)(i,j);
    delete[] x_;x_=x_new;m_=m_new;n_=n_new;delete Q_;delete V_;delete R_;Q_=V_=0;R_=0;}
	...
	inline void Transposed_Multiply(const VectorND<T>& y,VectorND<T>& result)//result=(*this).Transpose()*y
    {assert(y.n==m_ && result.n==n_);for(int i=0;i<m_;i++) for(int j=0;j<n_;j++) result(j)+=*(x_+(j)*m_+i)*y(i);}

	inline VectorND<T> Transposed_Multiply(const VectorND<T>& y) const //result=(*this).Transpose()*y
	{
		assert(y.num_dimension_==m_);
	
		VectorND<T> result;
		result.Initialize(n_,true);
	...
	inline void Multiply(const VectorND<T>& y,VectorND<T>& result)
    {assert(y.n==n_ && result.n==m_);for(int j=0;j<n_;j++) for(int i=0;i<m_;i++) result(i)+=*(x_+(j-1)*m_+i-1)*y(j);}
    ...
    {int size=m_*n_;for(int i=0;i<size;i++)x_[i]=0;}
    ...
    {MATRIX_NXN<T> A_transpose_A(Normal_Equations_Matrix());VectorND<T> A_transpose_b(Transpose_Times(b));return A_transpose_A.Cholesky_Solve(A_transpose_b);}
