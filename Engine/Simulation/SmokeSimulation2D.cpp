...
void SmokeSimulation2D::initialize()
{
	ScriptReader script;
...
void SmokeSimulation2D::initializeSourceListFromScript(ScriptBlock& super_block)
{
...
void SmokeSimulation2D::initializeObjectListFromScript(ScriptBlock& super_block)
{
...
void SmokeSimulation2D::initializeLinearSystem(MT* mt, const int& thread_id, const GridUniform3D& grid_ghost, Array3D<int>& boundary_array, CSRMatrix<D>& A_matrix, VectorND<D>& x_vector, VectorND<D>& b_vector)
{
	int full_ix(0);
...
TV SmokeSimulation2D::getCollisionVelocity(const TV& vel, const TV& normal, const T& res, const T& fric_dt)
{
...
void SmokeSimulation2D::setupBoundaryCondition(MT* mt, const int& thread_id)
{
	// smoke
	// 	BEGIN_BOX_ITERATION_ARR_3D_SYNC(grid_ghost_)
	// 	{
...
void SmokeSimulation2D::project(MT* mt, const int thread_id, const T dt)
{
...
void SmokeSimulation2D::solveLinearSystemCPUCG(MT* mt, const int& thread_id, CSRMatrix<D>& A_matrix, VectorND<D>& x_vector, VectorND<D>& b_vector)
{
	BEGIN_ONE_THREAD_WORK
	{
...
void SmokeSimulation2D::updateVelocityByPressureGradient(MT* mt, const int& thread_id)
{
...
void SmokeSimulation2D::setupLinearSystem(MT* mt, const int& thread_id, const GridUniform3D& grid_ghost, Array3D<int>& boundary_array, Array3D<T>& divergence_array, Array3D<T>& pressure_array)
{
	BEGIN_ONE_THREAD_WORK
	{
...
void SmokeSimulation2D::updateFaceVelocityHelper(const TV& particle_pos, const T& vel_compoment, const GridUniform3D& face_grid, const int &i, const int& j, const int& k, const Array3D<T>& face_vel_array, const Array3D<T>& face_vel_array_temp)
{
...
void SmokeSimulation2D::addNewParticlesFromSourceList(MT* mt, const int& thread_id)
{
	//	for (int oix = 0; oix < fixed_solid_list_.num_elements_; oix++)
	{
...
void SmokeSimulation2D::getGridVelocityFromParticles(MT* mt, const int& thread_id)
{
...
void SmokeSimulation2D::advectParticles(MT* mt, const int& thread_id)
{
...
void SmokeSimulation2D::advectEulerianVelocity(MT* mt, const int& thread_id)
{
	// Eulerian advection
	BEGIN_BOX_ITERATION_ARR_3D_SYNC(grid_ghost_)
	{
...
void SmokeSimulation2D::saveVelocity(MT* mt, const int& thread_id)
{
...
void SmokeSimulation2D::saveVelocityDifference(MT* mt, const int& thread_id)
{
	BEGIN_1D_ITERATION(u_velocity_array_temp_.ijk_res_)
	{
...
void SmokeSimulation2D::updateOneStep(MT* mt, const int thread_id)
{
	// advance one frame
	const T dt = 0.001f;
...
void SmokeSimulation2D::advanceOneTimeStep(MT* mt, const int thread_id, const T dt)
{
...
void SmokeSimulation2D::updateDataDepot(MT* mt, const int thread_id, DataDepot* data_depot)
{
	BEGIN_ONE_THREAD_WORK
	{
		data_depot->reset();
