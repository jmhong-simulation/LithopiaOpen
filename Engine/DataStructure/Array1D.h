...
//		assert(i < num_elements_);
	...
	void compactArray(Array1D<int>& flag_array, const int skip_flag)
	{
		// copy num of valid elements
		int new_num_elements = 0;
		for (int i = 0; i < num_elements_; i++)
		{
			if (flag_array[i] != skip_flag) flag_array[i] = new_num_elements++;
		}
